<#
.Synopsis
    Deploys an Elasticsearch cluster with Transport Layer Security to Kibana
.Example
    & .\encrypted_communication_kibana.ps1 -AdminUserName "russ" `
    -AdminPassword $(ConvertTo-SecureString -String "Password1234" -AsPlainText -Force) `
    -CertPath "C:\cert.crt" -KeyPath "C:\cert.key" `
...
.Example
    & .\encrypted_communication_kibana.ps1 -ClientId "clientid" `
...
.Parameter CertPath
    the path to the certificate to use to encrypt communication with Kibana
.Parameter KeyPath
    the path to the private key to use to encrypt communication with Kibana
...
$resourceGroup = "encrypted-kibana-cluster"
$name = "elasticsearch"
$kibanaCert = [Convert]::ToBase64String([System.IO.File]::ReadAllBytes($CertPath))
$kibanaKey = [Convert]::ToBase64String([System.IO.File]::ReadAllBytes($KeyPath))
...
$deployment = New-AzureRmResourceGroupDeployment -Name $name -ResourceGroupName $resourceGroup `
